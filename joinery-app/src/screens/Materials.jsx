...
function AddMaterialModal({ targetCat, cols, allCats, material, onClose, onCreated, onUpdated, onDeleted }) {
  ...
  // Whenever the preferred supplier (or its price) changes, mirror that price onto
  // the material's main `price` field so order sheets / pickers always show the right number
  function syncPreferredPrice(link) {
    if (link?.price !== undefined && link.price !== '') {
      setForm(p => ({ ...p, price: link.price }))
      if (material?.id) {
        supabase.from('materials').update({ price: link.price }).eq('id', material.id)
        ...
        {modalTab === 'suppliers' && isEditing ? (
          <div style={{ flex:1, overflowY:'auto', padding:20 }}>
            ...
            <div style={{ fontSize:11, color:'#9CA3AF', marginTop:14, lineHeight:1.5 }}>
              The price shown on order sheets and material pickers uses the <strong>preferred</strong> supplier's price.
            </div>
          </div>
        ) : (
        <div style={{ flex:1, overflowY:'auto', padding:20 }}>
          ...
          {/* Price + breaks — not shown for kits, price is derived from components above */}
          {!isKit && (
          <div style={{ background:'#F8FAFF', border:'1px solid #E0E7FF', borderRadius:12, padding:14 }}>
            ...
            <div style={{ fontSize:10, fontWeight:700, color:'#6B7280', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6 }}>Qty price breaks (optional)</div>
            {priceBreaks.map((b, i) => (
              <div key={i} style={{ display:'flex', gap:6, marginBottom:6, alignItems:'center' }}>
                <span style={{ fontSize:11, color:'#9CA3AF', flexShrink:0 }}>≥</span>
                <input type="number" placeholder="Qty" value={b.qty}
                  onChange={e=>setPriceBreaks(p=>p.map((x,j)=>j===i?{...x,qty:e.target.value}:x))}
                  style={{ width:70, padding:'6px 8px', border:'1px solid #DDE3EC', borderRadius:7, fontSize:12, outline:'none' }} />
                <span style={{ fontSize:11, color:'#9CA3AF', flexShrink:0 }}>→ $</span>
                <input type="number" placeholder="0.00" value={b.price}
                  onChange={e=>setPriceBreaks(p=>p.map((x,j)=>j===i?{...x,price:e.target.value}:x))}
                  style={{ flex:1, padding:'6px 8px', border:'1px solid #DDE3EC', borderRadius:7, fontSize:12, outline:'none' }} />
                <button onClick={()=>setPriceBreaks(p=>p.filter((_,j)=>j!==i))}
                  style={{ background:'none', border:'none', cursor:'pointer', color:'#D1D5DB', fontSize:14 }}>×</button>
              </div>
            ))}
            <button onClick={()=>setPriceBreaks(p=>[...p,{qty:'',price:''}])}
              style={{ width:'100%', padding:'6px', borderRadius:7, border:'1px dashed #C7D2FE', background:'none', color:'#5B8AF0', fontSize:11, fontWeight:600, cursor:'pointer' }}>
              + Add price break
            </button>
          </div>
          )}
