...
export default function InlineSpecBuilder({ specId, jobId, onBack }) {
  ...
  async function downloadPDF() {
    ...
    function itemImg(item) {
      ...
      return url ? `<img src="${url}" style="width:52px;height:52px;object-fit:cover;border-radius:8px;border:1px solid #ddd;flex-shrink:0;display:block" crossorigin="anonymous" />` : ''
    }
    function matRow(m) {
      const tags = [m.supplier,m.panel_type,m.thickness?m.thickness+'mm':null,m.finish].filter(Boolean).join(' · ')
      ...
      return `<div style="display:flex;gap:12px;align-items:flex-start;padding:10px 12px;background:#f8f8f8;border-radius:8px;border:1px solid #e0e0e0;margin-bottom:8px">${swatch}<div style="flex:1"><div style="font-size:14px;font-weight:600;color:#000">${m.name||'—'}</div>${tags?`<div style="font-size:11px;color:#444;margin-top:2px">${tags}</div>`:''} ${m._note?`<div style="margin-top:6px;font-size:12px;color:#000;background:#e8eeff;border-left:2px solid #5B8AF0;border-radius:4px;padding:5px 8px">${m._note}</div>`:''}</div></div>`
    }
    function appRow(a) {
      ...
      return `<div style="display:flex;gap:12px;align-items:flex-start;padding:10px 12px;background:#f8f8f8;border-radius:8px;border:1px solid #e0e0e0;margin-bottom:8px">${swatch}<div style="flex:1"><div style="font-size:14px;font-weight:600;color:#000">${[a.brand,a.model].filter(Boolean).join(' ')||'—'}</div>${sub?`<div style="font-size:11px;color:#444;margin-top:2px">${sub}</div>`:''} ${a._note?`<div style="margin-top:6px;font-size:12px;color:#000;background:#e8eeff;border-left:2px solid #5B8AF0;border-radius:4px;padding:5px 8px">${a._note}</div>`:''}</div></div>`
    }
    const DIM_LABELS={toe_kick_height:'Toe kick',base_height:'Base height',base_depth:'Base depth',upper_height:'Upper height',upper_depth:'Upper depth',tall_height:'Tall height',tall_depth:'Tall depth',bench_thickness:'Bench thickness',bench_overhang_front:'Overhang front',bench_overhang_side:'Overhang sides',bench_material:'Bench material'}
    function itemsByStatus(arr){const g={confirmed:[],maybe:[],alternative:[]};arr.forEach(e=>{const s=e._status||'confirmed';(g[s]||g.confirmed).push(e)});return g}
    function statusSection(label,color,items,rowFn){if(!items.length)return '';return `<div style="margin-bottom:10px"><div style="font-size:10px;font-weight:700;color:${color};text-transform:uppercase;letter-spacing:.05em;margin:10px 0 5px">${label}</div>${items.map(rowFn).join('')}</div>`}
    const roomSections=rooms.map(room=>{
      ...
      const mg=itemsByStatus(mats),ag=itemsByStatus(apps)
      return `<div style="margin-bottom:32px;page-break-inside:avoid"><div style="display:flex;align-items:baseline;justify-content:space-between;padding-bottom:8px;border-bottom:2px solid #000;margin-bottom:14px"><div style="font-size:18px;font-weight:700;color:#000">${room.name}</div><div style="font-size:12px;color:#444">${mats.length} material${mats.length!==1?'s':''} · ${apps.length} appliance${apps.length!==1?'s':''}</div></div>${hasDims?`<div style="font-size:10px;font-weight:700;color:#444;text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">Dimensions</div><div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:14px">${Object.entries(dims).filter(([,v])=>v).map(([k,v])=>`<div style="background:#f5f5f5;border:1px solid #e0e0e0;border-radius:6px;padding:5px 10px;font-size:12px"><span style="color:#555">${DIM_LABELS[k]||k}: </span><strong>${v}${k!=='bench_material'?'mm':''}</strong></div>`).join('')}</div>`:''}${mats.length>0?`<div style="font-size:10px;font-weight:700;color:#444;text-transform:uppercase;letter-spacing:.06em;margin:8px 0 4px">Materials</div>${statusSection('✓ Confirmed','#166534',mg.confirmed,matRow)}${statusSection('? Maybe','#854D0E',mg.maybe,matRow)}${statusSection('≈ Alternatives','#3730A3',mg.alternative,matRow)}`:''}${apps.length>0?`<div style="font-size:10px;font-weight:700;color:#444;text-transform:uppercase;letter-spacing:.06em;margin:14px 0 4px">Appliances</div>${statusSection('✓ Confirmed','#166534',ag.confirmed,appRow)}${statusSection('? Maybe','#854D0E',ag.maybe,appRow)}${statusSection('≈ Alternatives','#3730A3',ag.alternative,appRow)}`:''} ${room.notes?`<div style="margin-top:10px;font-size:12px;color:#000;background:#fffbeb;border-left:3px solid #f59e0b;padding:8px 12px;border-radius:4px"><strong>Notes:</strong> ${room.notes}</div>`:''}</div>`
    }).join('')
    const totalMats=rooms.reduce((s,r)=>s+(r.materials||[]).length,0)
    const totalApps=rooms.reduce((s,r)=>s+(r.appliances||[]).length,0)
    const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${spec.title||'Spec'}</title><style>*{box-sizing:border-box;margin:0;padding:0;-webkit-print-color-adjust:exact;print-color-adjust:exact}body{font-family:-apple-system,BlinkMacSystemFont,'Helvetica Neue',Arial,sans-serif;color:#000;background:#fff;padding:32px 40px;max-width:800px;margin:0 auto}img{display:block}@media print{body{padding:0;max-width:100%}@page{margin:15mm;size:A4}}</style></head><body><div style="border-bottom:2px solid #000;padding-bottom:18px;margin-bottom:24px"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><div style="font-size:28px;font-weight:800;color:#000">${spec.title||'Untitled Spec'}</div>${spec.client?`<div style="font-size:15px;color:#333;margin-top:4px">${spec.client}</div>`:''}</div><div style="text-align:right"><div style="display:inline-block;font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;background:#e8e8e8;color:#000;margin-bottom:6px">${spec.status||'Draft'}</div><div style="font-size:12px;color:#333">${date}</div></div></div><div style="display:flex;gap:28px;margin-top:16px"><div><span style="font-size:20px;font-weight:800;color:#000">${rooms.length}</span><span style="font-size:12px;color:#444;margin-left:4px">rooms</span></div><div><span style="font-size:20px;font-weight:800;color:#000">${totalMats}</span><span style="font-size:12px;color:#444;margin-left:4px">materials</span></div><div><span style="font-size:20px;font-weight:800;color:#000">${totalApps}</span><span style="font-size:12px;color:#444;margin-left:4px">appliances</span></div></div></div>${roomSections}<div style="margin-top:32px;padding-top:14px;border-top:1px solid #ddd;display:flex;justify-content:space-between;font-size:11px;color:#666"><span>${spec.title||'Spec'}</span><span>${date}</span></div></body></html>`
    const win=window.open('','_blank');win.document.write(html);win.document.close()
    win.addEventListener('load',()=>setTimeout(()=>win.print(),300));setTimeout(()=>win.print(),1200)
  }
  ...
  return (
    <div>
      ...
      {/* Add room */}
      {showAddRoom && (
        <div style={{ display:'flex', gap:8, marginBottom:14, padding:'12px 14px', background:'#F0F4FF', borderRadius:10, border:'1px solid #C4D4F8' }}>
          <input autoFocus value={newRoomName} onChange={e=>setNewRoomName(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addRoom()}
            placeholder="Room name…" style={{ flex:1, padding:'7px 10px', border:'1px solid #DDE3EC', borderRadius:8, fontSize:13, outline:'none' }} />
          <button onClick={addRoom} disabled={!newRoomName.trim()} style={{ fontSize:12, fontWeight:700, padding:'7px 14px', borderRadius:8, border:'none', background:'#5B8AF0', color:'#fff', cursor:'pointer' }}>Add</button>
          <button onClick={()=>{setShowAddRoom(false);setNewRoomName('')}} style={{ fontSize:12, fontWeight:600, padding:'7px 12px', borderRadius:8, border:'1px solid #E8ECF0', background:'#fff', cursor:'pointer', color:'#6B7280' }}>Cancel</button>
        </div>
      )}
